...
function getRelationshipsOfUser(req, res) {
	...
	// select username from users where id in (select followed_id from relationships where follower_id = 1);

	db.serialize(() => {
		db.all('SELECT username FROM users WHERE id IN(SELECT followed_id FROM relationships WHERE follower_id = ?)', [user_id], (err, rows) => {
